...
public class SMRepairExperiment 
{
	/**
	 * @param args
	 */
	public static void main(String[] args) 
	{
		...
		double warmUpPeriod=7*7*24*60; //7 weeks for warmup
		...
		for (numStaffA=9;numStaffA<=16;numStaffA++)
		{
			while(numStaffA<=16 && numStaffB<=30)
			{
				...
				for(i=0 ; i < NUMRUNS ; i++)
				{
					...
					cc.setTimef(warmUpPeriod);
					cc.runSimulation();
					cc.clearAllOutput();
					cc.setTimef(endTime);
					cc.runSimulation();
					printSSOVs(i+1,(endTime-warmUpPeriod)/(24.0*60.0), cc);
		...
		for (numStaffA=9;numStaffA<=16;numStaffA++)
		{
			while(numStaffA<=16 && numStaffB<=30)
			{
				...
				for(i=0 ; i < NUMRUNS ; i++)
				{
					cc = new CallCenter(startTime,endTime,dm,numStaffA,numStaffB,sds[i]);
					cc.setTimef(warmUpPeriod);
					cc.runSimulation();
					cc.clearAllOutput();
					cc.setTimef(endTime);
